...
void parse_get_request(cJSON *request, req_struct **reqObj, PAYLOAD_TYPE type)
{
	...
	if ( reqObj != NULL ) {
		...
		if((*reqObj)->u.getReq != NULL) {
			...
			WdmpError("Malloc is Failed\n");
		}
	}
	else {
		WdmpError("Empty regObj is received\n");
	}
}
...
void parse_test_and_set_request(cJSON *request, req_struct **reqObj)
{
	...
	if( reqObj != NULL ) {
		...
		if((*reqObj)->u.testSetReq != NULL) {
			memset((*reqObj)->u.testSetReq,0,(sizeof(test_set_req_t)));

			if(cJSON_GetObjectItem(request, "old-cid") != NULL)
			{
				(*reqObj)->u.testSetReq->oldCid = strndup(cJSON_GetObjectItem(request, "old-cid")->valuestring, MAX_PARAMETER_LEN);
				WdmpPrint("(*reqObj)->u.testSetReq->oldCid : %s\n",(*reqObj)->u.testSetReq->oldCid);
			}
			if(cJSON_GetObjectItem(request, "new-cid") != NULL)
			{
				(*reqObj)->u.testSetReq->newCid = strndup(cJSON_GetObjectItem(request, "new-cid")->valuestring, MAX_PARAMETER_LEN);
				WdmpPrint("(*reqObj)->u.testSetReq->newCid : %s\n",(*reqObj)->u.testSetReq->newCid);
			}
			if(cJSON_GetObjectItem(request, "sync-cmc") != NULL)
			{
				(*reqObj)->u.testSetReq->syncCmc = strndup(cJSON_GetObjectItem(request, "sync-cmc")->valuestring, MAX_PARAMETER_LEN);
			...
			if (cJSON_GetObjectItem(request, "parameters") != NULL) // No Parameters
			{
				...
				if ((*reqObj)->u.testSetReq->param != NULL) {
...
void parse_replace_rows_request(cJSON *request, req_struct **reqObj)
{
	...
	if(reqObj != NULL) {
		...
		if((*reqObj)->u.tableReq != NULL) {
			...
			if((*reqObj)->u.tableReq->rows != NULL) {
				...
				for ( i = 0 ; i < rowCnt ; i++)
				{
					...
					if(((*reqObj)->u.tableReq->rows[i].names != NULL) && ((*reqObj)->u.tableReq->rows[i].values != NULL)) {
	...
	if(reqObj != NULL) {
		...
		if((*reqObj)->u.tableReq != NULL ) {
			...
			if((*reqObj)->u.tableReq->rows != NULL) {
				...
				if(((*reqObj)->u.tableReq->rows->names != NULL) && ((*reqObj)->u.tableReq->rows->values != NULL)) {
					for ( i = 0 ; i < paramCount ; i++)
					{
